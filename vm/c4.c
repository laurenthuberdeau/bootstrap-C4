...
int *e, *le,  // current position in emitted code
    ...
    *sym,     // symbol table (simple list of identifiers)
    ...
    ops,      // print opcodes
    run,      // load and run a bytecode (.op) file
...
int main(signed argc, char **argv)
{
  ...
  if (argc > 0 && **argv == '-' && (*argv)[1] == 'r') { run = 1; --argc; ++argv; }
  if (argc > 0 && **argv == '-' && (*argv)[1] == 'p') { portable = 1; --argc; ++argv; }
  if (argc < 1) { printf("usage: c4 [-s] [-d] [-b] [-r] [-p] file ...\n"); return -1; }
  ...
  if (run) {
    // ----- load and run a bytecode (.op) image -----
    // Format: <data size>\n <escaped data>\n <opcodes>. The code starts with
    // the jump-to-main code, so execution simply starts at index 0.
    ...
    // <data size>
    i = 0; while (*p >= '0' && *p <= '9') i = i * 10 + *p++ - '0';
    ++p; // skip newline

    // <data>: decode i bytes ('\hh' hex escapes) into the data segment
    while (i > 0) {
      if (*p == '\\') {
        ++p;
        bt = (*p <= '9' ? *p - '0' : *p - 'a' + 10) * 16; ++p;
        bt = bt + (*p <= '9' ? *p - '0' : *p - 'a' + 10); ++p;
        *data++ = bt;
      }
      else *data++ = *p++;
      --i;
    }
    ++p; // skip newline after the data line

    // <opcodes>: load into the code segment starting at index 0
    e = estart - 1;
    pc = estart; // entry point is first instruction
    while (*p) {
      // match the 4-char mnemonic against the opcode table
      i = 0;
      while (i <= EXIT && memcmp(p, &"LEA ,IMM ,REF ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PSH ,"
                                    "OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,"
                                    "OPEN,READ,CLOS,PRTF,MALC,FREE,MSET,MCMP,EXIT,"[i * 5], 4)) i++;
      if (i > EXIT) { printf("unknown opcode in bytecode\n"); return -1; }
      *++e = i; p = p + 4;
      if (i <= ADJ) { // opcode carries an immediate operand
        while (*p == ' ') ++p;
        bt = 1; if (*p == '-') { bt = -1; ++p; }
        ty = 0; while (*p >= '0' && *p <= '9') ty = ty * 10 + *p++ - '0';
        *++e = ty * bt;
      }
    ...
    // Fill in the reserved entry trampoline (JMP <main>) and start there, so that
    // compile-and-run follows the same path (and cycle count) as running a .op file.
    *estart = JMP; estart[1] = idmain[Val]; // idmain[Val] is a word index rel. to estart
    pc = estart;
    if (src) return 0;
    ...
    if (ops) {
      // <data size> (the entry trampoline was already written above)
      printf("%d\n", data - datastart);
      // <data>
