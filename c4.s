...
# The original C code is interleaved as comments; each C statement maps
# to the instruction sequence directly below it.
#
...
.bss
.align 8
p:      .space 8    # char *p;    current position in source code
lp:     .space 8    # char *lp;
data:   .space 8    # char *data; data/bss pointer
e:      .space 8    # int *e;     current position in emitted code
le_:     .space 8    # int *le;
id:     .space 8    # int *id;    currently parsed identifier
sym:    .space 8    # int *sym;   symbol table (simple list of identifiers)
tk:     .space 8    # int tk;     current token
ival:   .space 8    # int ival;   current token value
ty:     .space 8    # int ty;     current expression type
loc:    .space 8    # int loc;    local variable offset
line:   .space 8    # int line;   current line number
src:    .space 8    # int src;    print source and assembly flag
debug:  .space 8    # int debug;  print executed instructions
